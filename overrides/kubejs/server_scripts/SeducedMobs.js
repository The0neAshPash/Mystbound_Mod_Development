...
seducedMobs.forEach(seducedMob => {
   EntityJSEvents.addGoalSelectors(seducedMob, event => {
    event.customGoal('sirenSeduced', 1, m => m.target != null, m => m.target != null, true, m => { }, m => { }, true, mob => {
        ...
        if (!target.isPlayer()) return; // moved UP — bail before touching .player

        let nbt = target.getPersistentData(); // safer than target.player.getNbt()
        if (!nbt) return;

        let origin = nbt.getCompound('ForgeCaps')
            ?.getCompound('origins:origins')
            ?.getCompound('Origins')
            ?.getString('origins:origin')

        if (!origin || origin != 'mystbound-origins:siren') return;
